...
if (isset($_FILES['documento']) && $_FILES['documento']['error'] == 0) {
    ...
    if (in_array($fileExtension, $allowedExtensions)) {
        $newFileName = 'documento_' . $idusuario . '.' . $fileExtension;
        ...
        if (move_uploaded_file($fileTmpPath, $dest_path)) {
            $documentUrl = '../views/templates/assets/uploads/' . $newFileName;

            $consultas = new Consultas($conn);
            $resultado = $consultas->actualizarDocumento($documentUrl, $idusuario);
            ...
            if ($resultado) {
                ...
                echo "<script>console.log('Error al actualizar el documento en la base de datos.');</script>";
            }
        } else {
