...
<body class="vertical light">
  ...
  <script>
    $(document).ready(function() {
      $('#formincidencias').on('submit', function(event) {
        event.preventDefault(); // Prevenir el envío normal del formulario

        // Realizar la solicitud AJAX
        $.ajax({
          url: "../../models/insert.php", // Ruta a tu archivo PHP de procesamiento
          type: "POST",
          data: $(this).serialize(),
          success: function(response) {
            // Verificar si el registro fue exitoso
            if (response === 'success') { // Asegúrate que 'success' sea la respuesta esperada
              Swal.fire({
                icon: 'success',
                title: '¡Incidencia registrada!',
                text: 'La incidencia se ha registrado con éxito.',
                confirmButtonText: 'Aceptar'
              });
              // Opcionalmente, aquí puedes resetear el formulario si es necesario
              $('#formincidencias')[0].reset();
            } else {
              // Mostrar mensaje de error si la respuesta es diferente a 'success'
              Swal.fire({
                icon: 'error',
                title: 'Error',
                text: 'Hubo un problema al registrar la incidencia. Inténtalo de nuevo.',
                confirmButtonText: 'Aceptar'
              });
            }
          }
